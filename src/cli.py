...
def sync_push(session_id, repo, output, compress):
    ...
    try:
        ...
        # Sem session_id: listar sessões do projeto atual e deixar escolher
        ...
        # Nome padrão inclui o session-id para evitar colisão entre projetos
        if output is None:
            output = f"{session_id}.bundle"

        # 1. Buscar metadados para label descritivo no commit
        label = ""
        ...
        if project_dir:
            ...
            if meta:
                ...
                if project_path:
                    # Extrai só o nome do projeto (último segmento do caminho normalizado)
...
@click.option('--project-path', default=None,
              help='Local project path on this device (default: current directory)')
def sync_pull(session_id_prefix, repo, force, project_path):
    ...
    try:
        ...
        # Sem session_id_prefix: listar bundles disponíveis e deixar escolher
        ...
        # 1. Localizar bundle no repo (ensure_repo já foi chamado em list_bundles se necessário)
        bundle_path = git_sync.pull_bundle(session_id_prefix)

        if bundle_path is None:
            click.echo(f"[ERROR] No bundle found for session prefix '{session_id_prefix}'", err=True)
            click.echo(f"\nAvailable bundles:")
